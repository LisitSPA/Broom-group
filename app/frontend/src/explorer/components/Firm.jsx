  ...
  return (
    <ul>
      {subLevels.map((subLevel, index) => (
        <li key={index}>
          <div
            style={{ borderBottom: "3px solid #fff", backgroundColor: getFillColor(subLevel.level) }}
            className="flex items-center gap-5 justify-between px-5 text-xs"
          >
            <p>{subLevel.investorIdt} - {subLevel.societyNameh}</p>
...
const translateLevels = (ownersMap, firms, response) => {
  ...
  for (const vertex in adjacencyList) {
    ...
    if (typeof adjacencyList[vertex] === 'object' && !Array.isArray(adjacencyList[vertex])) {
      ...
      for (const neighbor in adjacencyList[vertex]) {
        ...
        porcentajes.push({ id: neighbor, porcentaje: percentage, padre: vertex  });
    ...
    levelSocietiesInfo.forEach((levelInfo, outerIndex) => {
      levelInfo.societies.forEach((societyInfo, innerIndex) => {
        ...
        if (societyInfo.investorId) {
           padre = buscarPadre(societyInfo.societyId, societyInfo.investorId, porcentajes);
...
const highestLevel = Math.max(...Object.keys(groupedByLevelAndSocietyIdt).map(Number));
...
/*Object.keys(groupedByLevelAndSocietyIdt).forEach((level) => {
  ...
  function buscarPadre(idPadre, idHijo, datos) {
    ...
    const nodosUnicos = Array.from(new Set(datos.map(JSON.stringify))).map(JSON.parse);
  ...
  useEffect(() => {
    ...
    handleOpenToggle();
  }, [isOpen, response, firmId, dispatch, firms]);
...
const renderLevels = (info, currentLevel, getLevelClassName) => (
  <div key={currentLevel} className={getLevelClassName(currentLevel)}>
    ...
    {Object.keys(info).map((societyIdt) => (
      <div key={societyIdt}>
        <p>Sociedad ID: {societyIdt} - Nombres: {info[societyIdt][0].societyNamet} - Rut: {info[societyIdt][0].rutsociedadt}</p>
        <ul>
          {info[societyIdt].map((item, index) => (
            <li key={index}>
              <div style={{ borderBottom: "3px solid #fff" }} className="flex items-center gap-5 justify-between px-5 text-xs">
                <p>{item.societies.investorIdt}- {item.societies.societyNameh}</p>
                <p>{item.societies.porcentaje}</p>
                <p>{item.societies.rutinv}</p>
              </div>
              {item.children && renderLevels(item.children, currentLevel + 1, getLevelClassName)}
            </li>
          ))}
        </ul>
      </div>
                        ...
                        {level === highestLevel.toString() && (
                          <div>
                            ...
                            {Object.keys(groupedInfo[level]).map((societyIdt) => (
                              <div key={societyIdt}>
                                <p>Sociedad ID: {societyIdt} - Nombres: {groupedInfo[level][societyIdt][0].societyNamet} - Rut: {groupedInfo[level][societyIdt][0].rutsociedadt}</p>
                                <ul>
